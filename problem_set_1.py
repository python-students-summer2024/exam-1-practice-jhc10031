...
def bark():
  ...
  message = "{}, the {} year old {}, says, 'Woof!'".format(name, age, breed) 
...
def bark_with_validation():
  ...
  if len(name) < 2:
    print("Name error!")
  elif 0 <= int(age) <= 15:  # is this incorrect? why does it keep telling me Age error! even if i put something within the range
    print("Age error!")
  elif breed != "Schnauzer" and breed != "Terrier" and breed != "Poodle" and breed !="Mastiff":
    print("Breed error!")
  else:
    message = "{}, the {} year old {}, says, 'Woof!'".format(name, age, breed) 
    print(message)
...
def respond_to_anything_but_nonsense():
  ...
  if sentence.find("nonsense") == True:  #confused about this
    return
